...
export default function CelebrityProfile() {
  ...
  const gridPosts = useMemo(() => {
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      return {
        id:      `gp_${i}`,
        image:   gridPhotos[i] ?? celeb.image,
        isVideo: false,
  ...
  if (!celeb && celebrities.length === 0) {
    return (
      <div style={{ minHeight:'100vh', background:'#000', display:'flex', alignItems:'center', justifyContent:'center' }}>
        <div style={{ width:32, height:32, border:'2px solid #333', borderTopColor:'#3b82f6', borderRadius:'50%', animation:'spin 0.7s linear infinite' }} />
      ...
      {!user && !isMemorial && (
        <div style={{ background:'linear-gradient(90deg,#1e3a5f,#2d1b69)', borderBottom:'1px solid #1a3a6f', padding:'12px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, flexWrap:'wrap' }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            <Link to="/signup" style={{ padding:'8px 18px', background:'#3b82f6', color:'#fff', borderRadius:8, fontWeight:700, fontSize:13, textDecoration:'none' }}>
              Sign up free
            </Link>
        ...
        <div style={{ padding:'20px 16px 0' }}>
          <div style={{ display:'flex', gap:16, alignItems:'flex-start', marginBottom:16 }}>
            ...
            <div style={{ position:'relative', flexShrink:0 }}>
              <div style={{ width:90, height:90, borderRadius:'50%', padding:3, background: isMemorial ? 'linear-gradient(135deg,#92400e,#fbbf24)' : 'linear-gradient(135deg,#3b82f6,#8b5cf6)' }}>
              ...
              {isMemorial ? (
                ...
                <div style={{ position:'absolute', bottom:2, right:2, width:22, height:22, borderRadius:'50%', background:'#3b82f6', border:'2px solid #000', display:'flex', alignItems:'center', justifyContent:'center' }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:12, color:'#3b82f6', fontWeight:600, marginBottom:10 }}>{celeb.category}</div>
              ...
              <div style={{ display:'flex', gap:8 }}>
                <button onClick={() => {
                  ...
                  background: following ? '#1a1a1a' : (isMemorial ? '#92400e' : '#3b82f6'),
              ...
              {followPrompt && !isMemorial && (
                <div style={{
                  ...
                  border:'1px solid #3b82f640', borderRadius:10,
                  ...
                  <div style={{ display:'flex', gap:8, flexShrink:0 }}>
                    <button onClick={() => navigate(`/messages?with=celeb_${celeb.id}`)} style={{
                      padding:'7px 14px', background:'#3b82f6', border:'none', borderRadius:7,
          ...
          {(wikiBio || celeb.bio) && (
            <p style={{ fontSize:13, color:'#999', lineHeight:1.6, marginBottom:14, paddingBottom:14, borderBottom:'1px solid #111' }}>
              ...
              {(wikiBio || celeb.bio).length > 260 ? (
                <span style={{ color:'#3b82f6', cursor:'pointer', marginLeft:4 }} onClick={() => setTab('about')}>…more</span>
              ) : ''}
            </p>
          )}
        ...
        {tab === 'posts' && (
          <>
            ...
            {!photosLoading && (
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:2 }}>
                {gridPosts.map(p => (
                  <GridCell key={p.id} post={p} celebMain={celeb.image} onExpand={setExpandedPost} />
                ))}
              </div>
            )}
          </>
        ...
        {tab === 'about' && (
          <div style={{ padding:'20px 16px' }}>
                ...
                <div style={{ fontSize:14, color:'#aaa', lineHeight:1.8 }}>
                  ...
                  {wikiBio && (
                    <a href={`https://en.wikipedia.org/wiki/${encodeURIComponent(celeb.name)}`}
                      ...
                      style={{ fontSize:12, color:'#3b82f6', textDecoration:'none', display:'inline-flex', alignItems:'center', gap:4, marginTop:4 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
              ...
              <button onClick={() => setShowFollowers(true)} style={{ background:'none', border:'none', cursor:'pointer', color:'#3b82f6', fontSize:13, fontWeight:600, fontFamily:'inherit' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
              {fakeFans.slice(0, 20).map(fan => (
                <Link key={fan.id} to={`/fan/${fan.id}`}
                  ...
                  <span style={{ fontSize:11, color:'#3b82f6', fontWeight:600, flexShrink:0 }}>View →</span>
                </Link>
              ))}
